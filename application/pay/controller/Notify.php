        ...
        try{
            ...
            {	$user = Db::name('User')->where('user_id',$list['upgrade_uid'])->find();
				...
				if($user['user_pid'] !=0 ){
					...
					if(!$resulta){
						return json(['error'=>1,'msg'=>'特推代理分润计算错误']);
					}
					$MoneyTagnt  = $upgrade_money  - $resulta;
					if($MoneyTagnt < 0){
						return json(['error'=>1,'msg'=>'特推代理计算分润为负数']);
					}
					
					$user_agent_id = Db::name('User')->where(['user_id'=>$list['upgrade_uid'],'user_state'=>0])->value('user_agent_id');
               		$agentresult = $this->DistritAgent($user_agent_id,true,$MoneyTagnt,$result);
					
				}else{
                	$user_agent_id = Db::name('User')->where(['user_id'=>$list['upgrade_uid'],'user_state'=>0])->value('user_agent_id');
               		$agentresult = $this->DistritAgent($user_agent_id,true,$upgrade_money,$result);
                }

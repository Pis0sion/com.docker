...
class Articletype extends Base{
    ...
    public function add()
    {
        ...
        {
            $post = $_POST;
            if(Db::name('article_type')->insertGetId(['type_name'=>$post['type_name'],'type_time'=>time()]))
            {
    ...
    public function edit()
    {
        if($this->request->isPost())
        {
            ...
            if(Db::name('article_type')->where(['type_id'=>$post['id']])->update($data))
            {
                return json(['error'=>0,'msg'=>'修改成功']);
            }
            return json(['error'=>1,'msg'=>'修改错误']);
        }
        $article_type = Db::name('article_type')->where(['type_id'=>input('param.id')])->find();
        if(empty($article_type))
        {
            echo "无分类";
            exit();
        }
        $this->assign($article_type);
        return $this->fetch();
    }
}

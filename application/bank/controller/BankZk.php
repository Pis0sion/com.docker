...
class BankZk extends Base{
	...
	public function banklist($id,$page){
		// $id,
		$idarr[] = $id;
		$page = $page;
		$rsaUtils    = new RSAUtils(file_get_contents(Config::$publicKey), file_get_contents(Config::$privateKey));
	    $url		 = Config::$baseHost . "/credit/open/apply/card/channel";
	    $timestamp   = Config::getMillisecond();
	    $sign 		 = $rsaUtils->encrypt($timestamp);
		...
		$data = array();
		foreach($arr['result'] as $k=>$v){
			foreach ($v['bankCardStyleList'] as $bk=>$bv){
				$tag[] = $bv['name'];
 			}
			$data[] = [
				'ChannelId'		=> $v['stationChannelId'],
				'id'		    => $v['stationBankCardChannelId'],
		...
		return array('error'=>0,'msg'=>$arr['message'],'data'=>$data,'paging'=>$arr['paging']); 
	}
	
	public function info($id){
		 //= '1011874486158360577';
		$rsaUtils = new RSAUtils(file_get_contents(Config::$publicKey), file_get_contents(Config::$privateKey));
	    $url = Config::$baseHost . "/credit/open/apply/card/".$id;
	    $timestamp = Config::getMillisecond();
	    $sign = $rsaUtils->encrypt($timestamp);
		...
		$data['name'] 		 = $arr['result']['bankCard']['name'];
		$data['description'] = $arr['result']['bankCard']['description'];
		
		
		foreach ($arr['result']['bankCardAttrList'] as $ak=>$av){
			$data['AttrList'][] = [
				'name'=>$av['name'],
		...
		foreach ($arr['result']['bankCardImgList'] as $ik=>$iv){
			$data['ImgList'][] = $iv['urlPath'];
		}
		
		foreach ($arr['result']['bankCardStyleList'] as $sk=>$sv){
			$data['StyleList'][] = $sv['name'] ;
		}
		
		$data['bankId']    = $arr['result']['stationBankCardChannel']['bankId'];
		$data['id'] 	   = $arr['result']['stationBankCardChannel']['id'];//1011874486158360577
		$data['ChannelId'] = $arr['result']['stationBankCardChannel']['stationChannelId'];
	    return array('error'=>0,'msg'=>$arr['message'],'data'=>$data); 
	}	
	...
	public function bankdo($params){
		...
		$rsaUtils   = new RSAUtils(file_get_contents(Config::$publicKey), file_get_contents(Config::$privateKey));
	    $url 		= Config::$baseHost . "/credit/open/apply/card/save";
	    $timestamp  = Config::getMillisecond();
	    $sign		= $rsaUtils->encrypt($timestamp);
